...
const {
  createPromise, promiseResolve, promiseReject, isAsyncFunction
} = binding;
...
function promisify(orig) {
  ...
  if (orig[kCustomPromisifiedSymbol]) {
    return orig[kCustomPromisifiedSymbol];
  }

  if (isAsyncFunction(orig)) {
    return orig;
  }
  ...
  const fn = function(...args) {
    ...
    try {
      orig.call(this, ...args, (err, ...values) => {
        ...
        } else if (argumentNames !== undefined) {
          const obj = {};
  ...
  fn[kCustomPromisifiedSymbol] = fn;
